...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun 21 14:11:00 2022
...
def extract_tabacco():
    ...
    html_text = requests.get("https://fumermoinscher.com/categorie-produit/cigarettes/?orderby=popularity", headers={'User-Agent': 'Mozilla/5.0'}).text
    ...
    html_text = requests.get("https://fumermoinscher.com/categorie-produit/tabac/?orderby=popularity", headers={'User-Agent': 'Mozilla/5.0'}).text
...
def extract_household_tasks():
    ...
    product , mean_price = ['Lessive (€/L)' , 'Nettoyant vitre (€/L)' , 'Papier toilette (€/U)' , 'Eponge (€/U)'] , [round(stat.mean(lessive) , 2) , round(stat.mean(vitre) , 2) , round(stat.mean(WC) , 2) , round(stat.mean(eponge) , 2)]
...
def extract_hygiene():
    ...
    sites = ['https://www.carrefour.fr/p/gel-douche-fleur-de-cerisier-le-petit-marseillais-3574661550510?t=2421',
             ...
             'https://www.carrefour.fr/p/gel-douche-grenade-ushuaia-3600551033686?t=2421',

             'https://www.carrefour.fr/p/gel-douche-biome-protection-dermo-peau-sensible-sanex-8718951391901?t=2421',
    ...
    product , mean_price = ['Gel douche (€/L)' , 'Dentifrice (€/L)' , 'Protection hygiénique (€/U)' , 'Préservatifs (€/U)'] , [round(stat.mean(savon) , 2) , round(stat.mean(dentifrice) , 2) , round(stat.mean(tampons) , 2) , round(stat.mean(preservatifs) , 2)]
...
def extract_food():
    ...
    sites = ["https://www.carrefour.fr/p/huile-isio-4-isio-4-3265474000018",

               "https://www.carrefour.fr/p/huile-de-tournesol-carrefour-classic-3270190118862",

               "https://www.carrefour.fr/p/huile-de-tournesol-lesieur-3265471024086"]
...
def extract_medecine():
    ...
    product , mean_price = ['Paracetamol (boîte de 8 comprimés de 1000mg)' , 'Ibuprofène (boîte de 12 comprimés de 400mg)' , 'Désinfectants (125mL)'] , [round(stat.mean(paracetamol) , 2) , round(stat.mean(ibu) , 2) , round(stat.mean(desinfect) , 2)]
...
def concat():
    ...
    ##df = (pd.concat([extract_tabacco() , extract_household_tasks() , extract_hygiene() , extract_food() , extract_medecine()])).T
...
def decomposition_prix_valeur(df_init , df):
    ...
    df.to_excel('prix_conso' + str(datetime.date.today())[:10] + '.xlsx' )

    print('Fichier des prix Excel enregistré')
    
    new_df.to_excel('valeur_100_conso' + str(datetime.date.today())[:10] + '.xlsx' )

    print('Fichier Excel enregistré')
