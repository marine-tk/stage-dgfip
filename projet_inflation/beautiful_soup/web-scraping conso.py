...
path_prix = "/home/marinetk/prix/"
path_valeur100 = "/home/marinetk/valeur_100/"
...
def decomposition_prix_valeur(df_init , df):
    ...
    df.to_csv(path_prix+'prix' + datetime.today().strftime('%Y-%m-%d') + '.csv')
    print('Fichier des prix CSV enregistré')
        
    new_df.to_csv(path_valeur100+'valeur_100' + datetime.today().strftime('%Y-%m-%d') + '.csv')
    print('Fichier CSV enregistré')
   
def upload_to_bucket(blob_name, path_to_file, bucket_name):
    """ Transfert du fichier blob_name, issu du répertoire path_to_file dans le bucket bucket"""
...
def final_function():
  ...
  prix_du_jour = 'prix' + datetime.today().strftime('%Y-%m-%d') + '.csv'
  valeur100_du_jour = 'valeur_100' + datetime.today().strftime('%Y-%m-%d') + '.csv'

  # Création des fichiers CSV
  decomposition_prix_valeur(init_df(), concat())
